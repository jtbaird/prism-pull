...
def test_check_dates():
    ...
    assert result is None
    with pytest.raises(ValueError, match="Date must be between 0 and 30 for February in a leap year."):
        ps.check_dates(30, 2, 2020)
    with pytest.raises(ValueError, match="Date must be between 0 and 29 for February in non leap year."):
        ps.check_dates(29, 2, 2021)
    with pytest.raises(ValueError, match="Date must be between 0 and 32 for this month."):
        ps.check_dates(32, 10, 2021)
    with pytest.raises(ValueError, match="Date must be between 0 and 31 for this month."):
